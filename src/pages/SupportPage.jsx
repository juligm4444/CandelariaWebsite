    ...
    <div className="app-shell">
      ...
      <main className="support-main section-shell">
          ...
          <div className="support-plan-grid">
            {plans.map((plan, index) => (
              <article className="support-plan-card" key={`${plan.name}-${index}`}>
                <div className="support-plan-ordinal" aria-hidden="true">
                  {PLAN_ORDINALS[index % PLAN_ORDINALS.length]}
                </div>
                <h3>{plan.name}</h3>
        ...
        <section className="support-contribution">
          <div>
            <h2>
              {t('support.contribution.titleA')}{' '}
              <span>{t('support.contribution.titleB')}</span>
            </h2>
            <p>{t('support.contribution.body')}</p>
          </div>

          <div className="support-contribution-card">
            <span className="support-coming-soon-badge">{t('cart.purchasesUnavailable')}</span>
            ...
            <p className="support-contribution-hint">{t('support.contribution.safe')}</p>
          </div>
        </section>
